...
class _GamesState extends State<Games>{
  ...
  Future<List<Game>> populateGames () async {
    ...
    games;
    var finalRes;
    var res = await networking.Steam.getGameInfo(prefs.getString("steamAPIKey"), prefs.get("steamID"), context);// this should never fail
    try{
      var res2 = await networking.Steam.getGameInfo(prefs.getString("steamAPIKey"), prefs.get("partnerSteamID")??"76561198899898216", context);
                ...
                content: SingleChildScrollView(
                    child: ListBody(
                        children: <Widget>[
                          ...
                          Text('You may still use this page as normal, but the games avaialble to you will not be filtered by your partner\'s games.'),
                          Text('You are most likely rate-limited. Try again in a few minutes.'),
                          Text('\nFULL ERROR: \n' + e.toString()),
                        ]
                    )
                ),
                ...
                child: Column(
                    children: [
                      Container(
                        ...
                        //TODO: Change to icon
                        child: gameImage ?? const Icon(Icons.games, size: 100),
                      ),
                      Container(
                          margin: const EdgeInsets.all(10),
                          padding: const EdgeInsets.all(10),
                          ...
                          child: DropdownButtonHideUnderline(child: DropdownButton<Game>(
                              ...
                              onChanged: (Game? newValue) {
                                ...
                                gameImage= networking.Steam.getGameImage(newValue.steamID);
